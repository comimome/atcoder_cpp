...
void solve(){
    ...
    vc<bool> p(n+5,false);
    vc<int> c;
    FOR(i,1,n)c.push_back(10000*(100-a[i])+i);
    sort(ALL(c));
    REP(i,x)p[c[i]%10000]=true;
    c.clear();
    FOR(i,1,n){
        if(!p[i])c.push_back(10000*(100-b[i])+i);
    }
    sort(ALL(c));
    REP(i,y)p[c[i]%10000]=true;
    c.clear();
    FOR(i,1,n){
        if(!p[i])c.push_back(10000*(200-(a[i]+b[i]))+i);
    }
    sort(ALL(c));
    REP(i,z)p[c[i]%10000]=true;
    FOR(i,1,n){
        if(p[i])cout<<i<<endl;
    }
}
